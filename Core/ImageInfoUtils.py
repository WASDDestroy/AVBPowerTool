...
class ImageInfoUtils:
    ...
    def log_if_debug(self, log_level ="T",
        ...
        self.my_logger.log(log_level, log_str, self.TAG)
    ...
    def check_image_exists(self, image_info_list : list[str], image_dir = None):
        ...
        if set_existing == set_image_info_list:
            self.my_logger.log("D", str((True, "EQUAL", set_existing)), self.TAG)
            return True, "EQUAL", set_existing
        else:
            self.my_logger.log("D", str((False, set_existing.difference(set_image_info_list), set_existing, set_image_info_list)), self.TAG)
            if len(existing_images) > len(image_info_list):
                self.my_logger.log("W", "MORE", self.TAG)
                return False, "MORE", set_existing.difference(set_image_info_list), set_existing, set_image_info_list
            if len(existing_images) < len(image_info_list):
                self.my_logger.log("W", "LESS", self.TAG)
                return False, "LESS", set_existing.difference(set_image_info_list), set_existing, set_image_info_list
            else:
                self.my_logger.log("W", "DIFF", self.TAG)
    ...
    def __image_info_parser(self, image_info_string : str, is_vbmeta_image = False) -> dict:
        ...
        """
        if is_vbmeta_image:
            self.my_logger.log("I", "Processing a VBMeta image.", "imageInfoParser")
        ...
        with open(os.path.join(os.getcwd(),
            ...
            for j in image_info_dic:
                ...
                if j == "Public key (sha1)":
                    self.my_logger.log("T", "Public key SHA1: " + image_info_dic[j], "autoDetectKeyFile")
                    # Start auto-matching once found public key SHA1
                    for k in range(len(sha1_list)):
                        if sha1_list[k][1] == image_info_dic[j]:
                            self.my_logger.log("T", "Key file: " + sha1_list[k][0], "autoDetectKeyFile")
                            image_info_dic_copy["Public key file"] = sha1_list[k][0]
                            break
                    else:
                        self.my_logger.log("W", "No appropriate key file found for image!", self.TAG)
    ...
    def __read_image_info_batch(self, image_list : list[str]) -> dict:
        ...
        self.my_logger.log("I", "Image info extraction ended successfully.")
    ...
    def save_result_to_file(self, result_dict : dict) -> None:
        ...
        with open(os.path.join(os.getcwd(),
                               "Core",
                               "currentConfigs",
                               "imageInfo.json"), "w+") as my_file:
            my_file.write(json_string)
        self.my_logger.log("I", "Image info successfully written into "
