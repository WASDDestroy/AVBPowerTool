...
class ConfigManager:
    ...
    def get_new_config_name(self, current_name, prompt = None):
        """
        ...
        """
        print(t("config_manager.current_name", name=current_name))
        print(prompt or t("config_manager.rename_required_prompt"))
        count = 0
        while count < 3:
            new_config_name = input(t("config_manager.new_config_name"))
            if self.__is_config_available(new_config_name) and len(new_config_name) > 0:
                return new_config_name
            else:
                print(t("config_manager.illegal_config_name"))
                count += 1
        else:
            new_config_name = "AutoRenamedConfig_" + time.strftime("%Y-%m-%d-%H:%M:%S", time.localtime())
            print(t("config_manager.auto_name", name=new_config_name))
            return new_config_name
